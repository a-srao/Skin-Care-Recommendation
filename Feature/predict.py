...
class GNNSkinClassifier(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        super(GNNSkinClassifier, self).__init__()
...
def process():
    ...
    for segment_id in np.unique(segments):
        segment_pixels = np.where(segments == segment_id)
        node_features = np.concatenate([np.mean(image[segment_pixels], axis=0), glcm_features])
        G.add_node(segment_id, features=node_features)
    ...
    model = torch.load('pretrained_gnn_model.pt')

    # Evaluate the GNN model on the constructed graph
    model.eval()
    with torch.no_grad():
        output = model(data)
        predicted_label = output.max(dim=1)[1].item()

    print("GNN Predicted Label:", predicted_label)
